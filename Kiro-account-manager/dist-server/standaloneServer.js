...
const ACCOUNTS_FILE = process.env.ACCOUNTS_FILE || path_1.default.join(process.cwd(), 'accounts.json');
...
const server = new proxyServer_1.ProxyServer(config, {
    onRequest: (info) => {
...
if (loadedAccountsCount === 0 && fs_1.default.existsSync(ACCOUNTS_FILE)) {
    try {
        const content = fs_1.default.readFileSync(ACCOUNTS_FILE, 'utf-8');
